...
def test_create_post(api_client, user, category, tag):
    ...
    assert response.data["success"] is True
    assert response.data["message"] == "Post created succcessfully"

@pytest.mark.django_db
def test_unauthenticted_posts(request,category, tag):
    url = reverse("post")
    payload = {
...
# @pytest.mark.django_db
# def test_create_post_invalid_data(api_client, user, category, tag):
#     url = reverse("post")
#     payload = {
#         "title": 12345,  
#         "content": True,  
...
#     print("Invalid Data Errors:", response.data.get("errors"))

#     assert response.status_code == 400
#     assert "title" in response.data.get("errors", {})
#     assert "content" in response.data.get("errors", {})
#     assert "category_id" in response.data.get("errors", {})
